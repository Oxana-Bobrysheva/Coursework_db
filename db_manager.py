...
class DBManager:
    ...
    def insert_vacancy(self, vacancy: dict):
        ...
        vacancy dict keys:
        id, name, employer_id, salary_from, salary_to, salary_currency, area_name, url
        """
        with self.connection.cursor() as cursor:
            cursor.execute("""
                ...
                ON CONFLICT (id) DO NOTHING
            """, (
                vacancy['id'], vacancy['name'], vacancy['employer_id'],
                vacancy['salary_from'], vacancy['salary_to'], vacancy['salary_currency'],
                vacancy['area_name'], vacancy['url']
            ))
        self.connection.commit()
    ...
    def get_avg_salary(self):
        """
        Return average of (salary_from + salary_to)/2 for vacancies with both salaries set
        """
        with self.connection.cursor() as cursor:
            cursor.execute("""
    ...
    def get_vacancies_with_keyword(self, keyword: str):
        """
        Return vacancies where vacancy name contains the keyword (case-insensitive).
        Returns list of tuples (vacancy_name, company_name, salary_from, url)
        """
        with self.connection.cursor() as cursor:
            cursor.execute("""
